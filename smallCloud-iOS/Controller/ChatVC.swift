...
final class ChatVC: MessagesViewController {
    ...
    init(user: UserInfo, channel: Channel) {
        self.user = user
    ...
    private func setupMessageInputBar() {
        messageInputBar.inputTextView.tintColor = .priamry1
        messageInputBar.sendButton.setTitleColor(.priamry1, for: .normal)
...
extension ChatVC: MessagesDataSource {
    func currentSender() -> SenderType {
        return Sender(senderId: user.email!, displayName: user.name!)
    }
...
extension ChatVC: MessagesDisplayDelegate {
    ...
    func backgroundColor(for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) -> UIColor {
        return isFromCurrentSender(message: message) ? .systemBlue : .incomingMessageBackground
    }
